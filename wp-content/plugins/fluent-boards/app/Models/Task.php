...
class Task extends Model
{
    ...
    {
        return $this->hasMany(\FluentBoardsPro\App\Models\Attachment::class,
            'object_id', 'id')
    ...
    public static function adjustSubtaskCount($subTaskParentId)
    {
        ...
        $parentTask = Task::findOrFail($subTaskParentId);

        if ($parentTask->settings
            && array_key_exists('subtask_count', $parentTask->settings)
        ) {
            $parentTaskSettings   = $parentTask->settings;
            $count = $parentTaskSettings['subtask_count'] - 1 > 0 ?? 0;
            $parentTaskSettings['subtask_count'] = $count;
